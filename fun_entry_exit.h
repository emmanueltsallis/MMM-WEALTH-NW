...
CYCLE(cur1, "SECTORS")
{
	...
	CYCLE_SAFES( cur1, cur, "FIRMS" )							// use a robust cycle to delete objects
	{
     ...
     // EXIT CONDITION: Both low market share AND high debt required (changed from OR to AND)
     v[30] = VS(country, "exit_market_share_threshold");  // default 0.001
     v[31] = VS(country, "exit_debt_rate_threshold");     // default 1.5
     if(v[30] <= 0) v[30] = 0.001;  // fallback if parameter missing
     if(v[31] <= 0) v[31] = 1.5;    // fallback if parameter missing
     if ( v[1] <= v[30] && v[4] > v[31] && i > 1 && t>(v[0]+v[3]))
     {
      //plog("\nFirm Deleted - Low Market Share AND High Debt");
      	if (v[7]>=v[6])											//firm pays current debt with current deposits
      		{
      		v[11]=v[11]+(v[7]-v[6]);							//deposits distributed to income classes
      		...
      		{
       		v[11]=v[11];										//deposits distributed to income classes
      		v[12]=v[12]+v[2];									//available productive capacity
...
*/
    ...
    if(v[0]==1 & v[6]>0 && v[7]>0 && v[10]>v[11])  // Option A: removed v[8]>0 (3rd consecutive growth period)
		v[9]=1;
    else
		v[9]=0;
RESULT(v[9])
...
/*
EQUATION("Entry")
// SWITCHED: From Class_Stock_Deposits to Household_Stock_Deposits
v[26]=MAXLS(households, "Household_Stock_Deposits", 1);				//check max deposits among households
cur=SEARCH_CNDLS(households, "Household_Stock_Deposits", v[26], 1);
v[24]=VLS(cur, "Household_Stock_Deposits", 1);						//get deposits of household with max
