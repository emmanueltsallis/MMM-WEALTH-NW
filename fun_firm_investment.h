...
FIX: Added safeguard against explosive growth rates when older orders
are very small. This prevents inf values while logging when bounds are triggered
to help diagnose underlying demand volatility issues.
*/
	...
	{
		// Calculate growth rate with safeguard against extreme values
		v[5] = (v[1]-v[2])/v[2];                            //raw growth rate

		// Bound growth rate to [-0.9, 10] to prevent explosive expectations
		// -0.9 allows up to 90% decline, 10 allows 10x growth
		if(v[5] > 10 || v[5] < -0.9)
		{
			LOG("\n  [t=%d] WARNING: Firm expected sales growth rate bounded from %.2f to %.2f (orders: recent=%.2f, older=%.2f)",
				t, v[5], max(-0.9, min(10.0, v[5])), v[1], v[2]);
			v[5] = max(-0.9, min(10.0, v[5]));
		}

		v[4]=v[1]*(1+2*v[3]*v[5]);     		                //expected orders with bounded growth rate
	}
