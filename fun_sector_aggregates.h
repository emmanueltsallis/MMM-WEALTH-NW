...
EQUATION("Sector_Labor_Demand")
/*
Stage 5.2: Sectoral labor demand based on production requirements.
Labor Demand = Effective Production / Average Productivity

This is the SIGNAL for MARKOV employment transitions:
- Total Labor Demand > Actual Employment → Positive gap → Hiring
- Total Labor Demand < Actual Employment → Negative gap → Firing

Init safeguard (t<=2): Use lagged value to prevent mass firings during startup.
*/
v[0] = V("Sector_Effective_Production");
v[1] = V("Sector_Avg_Productivity");
v[2] = VL("Sector_Labor_Demand", 1);

// Production-based labor need
v[3] = (v[1] > 0) ? v[0] / v[1] : 0;

// Init safeguard: preserve employment during initialization
if(t <= 2)
    v[4] = (v[2] > 0) ? v[2] : v[3];
else
    v[4] = v[3];
